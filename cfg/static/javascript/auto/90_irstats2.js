...
var EPJS_Stats_GoogleGraph = Class.create(EPJS_Stats, {
	...
	initialize: function($super,params) {
		...
		var options = params.options;
        	...
        	// UZH CHANGE ZORA-498 Audit #5 2017/11/20/jv add <th> phrases
        	//
        	// depends on type of statistic: download or depostits get
        	// their own phrases by parameter; forward them to GoogleGraph 	
        	//
		if( params.options.graph_timeline == null )
		{
			alert( 'Missing param "graph_timeline" in EPJS_Stats' );
			return 0;
		}
		if( params.options.graph_total == null )
		{
			alert( 'Missing param "graph_total" in EPJS_Stats' );
			return 0;
		}
		if( params.options.graph_average == null )
		{
			alert( 'Missing param "graph_average" in EPJS_Stats' );
			return 0;
		}
		this.graph_timeline = options.graph_timeline;
		this.graph_total = options.graph_total;
		this.graph_average = options.graph_average;
        	// END UZH CHANGE ZORA-498 Audit #5 2017/11/20/jv add <th> phrases
	},
	...
	ajax: function($super,response) {
		...
		var data = new google.visualization.DataTable();
        	// UZH CHANGE ZORA-498 Audit #5 2017/11/20/jv add <th> phrases
		data.addColumn('string', this.graph_timeline);
		data.addColumn('number', this.graph_total);

		if( json.show_average )
			data.addColumn('number', this.graph_average);
        	// END UZH CHANGE ZORA-498 Audit #5 2017/11/20/jv add <th> phrases
...
var EPJS_Stats_GooglePieChart = Class.create(EPJS_Stats, {
	...
	ajax: function($super,response) {
		...
		//var options = {'width':w, 'height':h};
		// if ( typeof json.options.slices !== 'undefined' )
		//{
			var options = {'width':w, 'height':h, 'slices':json.options.slices};
		// }
...
var EPJS_Stats_Export_Toggle = function( el, content_id ) {
	...
	if( el != null && content != null )
	{
        ...
        // UZH CHANGE ZORA-506 2016-08-31 jv - toggle with new arrows
        updateWorkflowIcons();
